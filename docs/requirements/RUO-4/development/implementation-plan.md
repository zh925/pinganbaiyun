...
| 子任务 | Issue | 负责人 | 依赖 | 状态 |
|---|---|---|---|---|
| Android 工程、业务实现、协议与本地测试 | RUO-10 | 功能开发工程师 | 确认需求 merge commit | 已完成开发、自测 |
| 覆盖矩阵与独立验证 | RUO-10 | 测试与持续集成工程师 | 可测试实现 commit | 待独立验证 |
...
- JVM：字段规范化、16 位 key 拒绝、默认关系原子变更、重复 MAC、协议黄金向量、seed 边界、单任务锁、取消迟到回调、阶段超时和中性终态。
