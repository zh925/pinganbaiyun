...
- 当前执行状态：实现者已在集成测试计划前的 PR head `f7643f30e64eb86ddb22ebcfde018221a0a1939d` 完成本地 JVM、debug 构建与 lint 自检；独立验证尚未执行
...
| AC-12、NFR-07 | 故障注入单元 + 仪器 | 密文损坏、JSON 损坏、Keystore key 失效 | 注入不可解密/不可解析数据并启动列表 | 不崩溃、不覆盖原数据；仅经二次确认后安全重置 | 未执行 | 待故障注入与仪器验证 |
| AC-13～AC-15 | 状态机单元 + 假 GATT + 授权真机 | 已知 MAC 直连与 GATT 通道准备 | 用 fake 校验调用顺序；真机复核无 scan、目标 Service 与 descriptor 串行回调 | 单 taskId；不依赖 bluetoothName/scan；回调成功后才读 seed | 代码已实现并编译；假 GATT/真机未执行 | `AndroidGattTransport` |
...
| AC-17～AC-19 | 参数化单元 + 假 GATT | seed/DES/GATT 发起与回调失败 | 注入空 seed、>6 字节 seed、加密异常、API 未发起、非成功 status | 唯一失败终态；不发送空/部分帧；释放资源并给显式重试 | seed 边界 JVM 通过；假 GATT 未执行 | `SafeBaiyunProtocolTest` |
| AC-18 | 单元 + UI 文案 | 传输成功语义 | 注入 write callback `GATT_SUCCESS` | 仅显示“开门指令已发送，请确认门禁状态”，不宣称门已打开 | 实现者 JVM 通过；UI 未执行 | `UnlockCoordinatorTest` |
| AC-20～AC-25、NFR-01～NFR-03、NFR-11 | 状态机模型 + 虚拟时钟 + 并发仪器 | 单任务、取消/迟到回调、阶段超时、显式重试、资源释放 | 竞争手动/自动任务；在各阶段取消/超时并注入迟到回调；重复 50 轮 | 活跃任务 ≤1；每 taskId 恰一终态；无无限等待/自动重试/GATT 或计时器遗留 | 实现者 JVM 核心用例通过；仪器/50 轮未执行 | `UnlockCoordinatorTest` |
| AC-24、AC-30 | 单元 + 仪器 | 任务快照和重试读取最新配置 | 执行中编辑/删除配置，终态后显式重试 | 当前任务不变；重试读取最新配置；删除后不再自动触发 | 代码已实现；未执行 | 待独立验证 |
| AC-26～AC-30 | 生命周期仪器 | 冷启动一次、挂起续接、热启动/旋转/回前台不重触发 | `am force-stop` 后 Launcher 启动；权限/蓝牙页往返；旋转、重建、后台恢复 | 每次进程冷启动最多一个自动 taskId；其他生命周期事件仅恢复原任务 | 未执行 | 缺 Android 设备/模拟器运行证据 |
...
| AC-38 | 本地 Gradle | 单元测试与 debug 构建 | `./gradlew testDebugUnitTest assembleDebug` | 两项成功，协议、存储、调度、并发与错误测试全部通过 | 实现者本地通过（10 tests）；独立验证未执行 | PR head `f7643f3...` 的本地报告 |
...
实现者在 PR head `f7643f30e64eb86ddb22ebcfde018221a0a1939d` 执行上述三项均成功；独立验证必须在集成本计划后的新 head 重新执行并记录结果。CI/CD 未接入，GitHub 无检查不等于上述命令通过。
...
- 当前实现者执行的 10 个 JVM 测试均为确定性测试，未发现不稳定项；独立验证尚未确认。
